...
class _ScanReceiptPageState extends State<ScanReceiptPage> with AutomaticKeepAliveClientMixin {
    ...
    return Column(
      ...
      children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Scontrino elaborato con successo!',
                      style: TextStyle(
                        fontSize: 16,
        ...
        Row(
          children: [
            Expanded(
              child: _buildSecondaryButton('Riprova', Icons.refresh, () {
                setState(() {
                  _selectedImage = null;
                  _showResult = false;
                  _isProcessing = false;
  ...
  Widget _buildEditableField(String label, TextEditingController controller, IconData icon, bool isDark, {bool isAmount = false}) {
    return Container(
      ...
      child: TextField(
        ...
        keyboardType: isAmount ? TextInputType.numberWithOptions(decimal: true) : TextInputType.text,
        style: TextStyle(color: isDark ? Colors.white : const Color(0xFF1E293B), fontSize: 16, fontWeight: FontWeight.w500),
        decoration: InputDecoration(
          labelText: label,
          labelStyle: TextStyle(color: isDark ? Colors.white.withOpacity(0.6) : Colors.grey[600]),
          prefixIcon: Icon(icon, color: isDark ? Colors.white.withOpacity(0.6) : Colors.grey[600]),
    ...
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        decoration: InputDecoration(
          labelText: 'Categoria',
          labelStyle: TextStyle(color: isDark ? Colors.white.withOpacity(0.6) : Colors.grey[600]),
          prefixIcon: Icon(Icons.category, color: isDark ? Colors.white.withOpacity(0.6) : Colors.grey[600]),
          border: InputBorder.none,
          contentPadding: const EdgeInsets.all(16),
        ),
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Text(
                  'Data',
                  style: TextStyle(fontSize: 12, color: isDark ? Colors.white.withOpacity(0.6) : Colors.grey[600]),
                ),
                const SizedBox(height: 4),
                Text(
                  DateFormat('d MMMM yyyy', 'it_IT').format(_selectedDate),
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: isDark ? Colors.white : const Color(0xFF1E293B)),
                ),
              ],
            ),
            const Spacer(),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: isDark ? Colors.white.withOpacity(0.2) : Colors.grey[400]!),
        ),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, color: isDark ? Colors.white : Colors.grey[700], size: 18),
            const SizedBox(width: 8),
            Text(
              label,
              style: TextStyle(
  ...
  Future<void> _pickFromCamera() async {
    ...
    if (!granted) {
      _showSnackBar('Permesso fotocamera richiesto', const Color(0xFFEF4444));
      return;
    }
    try {
      ...
      _showSnackBar('Errore fotocamera: $e', const Color(0xFFEF4444));
    }
  }
  ...
  Future<void> _pickFromGallery() async {
    ...
    if (!granted) {
      _showSnackBar('Permesso galleria richiesto', const Color(0xFFEF4444));
      return;
    }
    try {
      ...
      _showSnackBar('Errore selezione: $e', const Color(0xFFEF4444));
    }
  }
  ...
  Future<void> _processWithAI() async {
    ...
    // Simulo elaborazione AI (sostituire con vera chiamata API)
    await Future.delayed(const Duration(seconds: 2));
    
    // Risultati AI simulati ma realistici
    ...
    _showSnackBar('Dati estratti automaticamente!', const Color(0xFF10B981));
  }
  ...
  void _saveTransaction() {
    ...
    final tx = MoneyTx(
      id: null,
      isIncome: false, // scontrini sono sempre spese

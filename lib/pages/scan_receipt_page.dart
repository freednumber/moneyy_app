...
class _ScanReceiptPageState extends State<ScanReceiptPage> with AutomaticKeepAliveClientMixin {
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Scansiona con OCA Space', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: isDark ? Colors.white : const Color(0xFF1E293B))),
              const SizedBox(height: 8),
              Text('OCR rapido tramite OCA Space (OCR.space)', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: isDark ? Colors.white.withOpacity(0.7) : Colors.grey[600])),
            ],
          ),
        ),
        const SizedBox(height: 24),
    ...
    return SizedBox(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: _isProcessing ? LinearGradient(colors: [Colors.grey.withOpacity(0.5), Colors.grey.withOpacity(0.3)]) : LinearGradient(colors: [const Color(0xFF6366F1), const Color(0xFF8B5CF6)]),
            borderRadius: BorderRadius.circular(16),
          ...
          child: Row(
            ...
            children: [
              if (_isProcessing) ...[
                ...
                const Text('Elaborazione OCA Space...', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
              ] else ...[
                const Icon(Icons.receipt_long, color: Colors.white, size: 20),
                const SizedBox(width: 8),
                const Text('Elabora con OCA Space', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
              ],
            ],
          ),
        ),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('OCR.space completato!', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1E293B))),
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Articoli Estratti (${_extractedItems.length})', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isDark ? Colors.white : const Color(0xFF1E293B))),
            ],
          ),
          ...
          ...(_extractedItems.take(5).map((item) => Padding(
            ...
            child: Row(
              children: [
                Expanded(child: Text('${item['quantity']}x ${item['name']}', style: TextStyle(fontSize: 13, color: isDark ? Colors.white.withOpacity(0.8) : Colors.grey[700]), maxLines: 1, overflow: TextOverflow.ellipsis)),
                Text('€${item['lineTotal'].toStringAsFixed(2)}', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: isDark ? Colors.white : const Color(0xFF1E293B))),
              ],
            ),
          )).toList()),
          if (_extractedItems.length > 5) Text('...e altri ${_extractedItems.length - 5} articoli', style: TextStyle(fontSize: 12, color: isDark ? Colors.white.withOpacity(0.5) : Colors.grey[500])),
        ],
      ),
    );
  }
  ...
  Future<void> _pickFromGallery() async {
    try {
      final image = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 85, maxWidth: 1920, maxHeight: 1920);

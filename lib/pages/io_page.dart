...
class _IOPageState extends State<IOPage> with TickerProviderStateMixin {
      ...
      child: Column(
        children: [
          ...
          if (_previewData.length > 50)
            Padding(
              padding: const EdgeInsets.all(16),
              child: Text('... e altre ${_previewData.length - 50} transazioni', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[600], fontStyle: FontStyle.italic)),
            ),
        ],
      ),
    );
          ...
          child: DropTarget(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _isDragging ? const Color(0xFF6366F1) : Colors.grey.withOpacity(0.4),
                  width: _isDragging ? 3 : 2,
                  style: BorderStyle.solid,
                ),
                boxShadow: [
                  if (_isDragging)
                    BoxShadow(color: const Color(0xFF6366F1).withOpacity(0.2), blurRadius: 12, spreadRadius: 2),
                ],
                ...
                children: [
                  AnimatedSwitcher(
                    ...
                    child: Icon(
                      ...
                      color: _isDragging ? const Color(0xFF6366F1) : Colors.grey,
                      key: ValueKey(_isDragging),
                    ),
                  ),
                  const SizedBox(height: 16),
                  Text(
                    _isDragging ? 'Rilascia il file qui!' : 'Trascina qui i tuoi file',
                    style: TextStyle(
                      fontSize: _isDragging ? 20 : 18,
                      fontWeight: _isDragging ? FontWeight.bold : FontWeight.w500,
                      color: _isDragging ? const Color(0xFF6366F1) : Colors.grey[700],
                    ),
                  ),
                  const SizedBox(height: 8),
                  Text('Supporta: CSV, Excel (.xlsx), MMBackup (.mmbackup)', style: TextStyle(fontSize: 14, color: Colors.grey[600])),
                  if (!_isDragging) ...[
                    const SizedBox(height: 16),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
                      decoration: BoxDecoration(color: const Color(0xFF6366F1).withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                      child: const Text('Oppure clicca "Sfoglia File" qui sotto', style: TextStyle(fontSize: 12, color: Color(0xFF6366F1), fontWeight: FontWeight.w500)),
                    ),
                  ],
                ],
              ),
  ...
  List<Map<String, String>> _generateCSVPreview() {
    ...
    for (int i = startIndex; i < lines.length && previewData.length < 100; i++) {
      ...
      try {
        ...
        if (fields.length >= 3) {
          previewData.add({'data': fields[0], 'categoria': fields.length > 1 ? fields[1] : 'N/A', 'importo': fields.length > 2 ? fields[2] : '0.00', 'nota': fields.length > 3 ? fields[3] : '', 'tipo': fields.length > 4 ? fields[4] : 'Uscita'});
        }
      } catch (_) {}
    }
    return previewData;
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            icon: _isAnalyzing ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.analytics),
  ...
  Future<void> _analyzeCSV(BuildContext dialogContext, String csvContent, MoneyModel model) async {
    if (csvContent.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Il contenuto CSV non può essere vuoto'), backgroundColor: Colors.red));
      return;
    }
    setState(() {
      _isAnalyzing = true;
    ...
    try {
      ...
      setState(() => _isAnalyzing = false);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Errore nell\'analisi del CSV: $e'), backgroundColor: Colors.red));
    }
  }
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: const Row(children: [Icon(Icons.warning, color: Colors.orange), SizedBox(width: 8), Expanded(child: Text('Trovate categorie non riconosciute', style: TextStyle(fontWeight: FontWeight.bold)))]),
        ),
        const SizedBox(height: 16),
        Row(children: [Icon(_getFileIcon(), color: Color(0xFF6366F1), size: 20), SizedBox(width: 8), Text('File: $_fileName', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500))]),
        const SizedBox(height: 8),
        ...
        ..._unrecognizedCategories.map((unknownCategory) {
          return Card(
            ...
            child: Padding(
              padding: const EdgeInsets.all(12),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Categoria trovata: "$unknownCategory"', style: const TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    ...
                    ...allCategories.map((cat) {
                      ...
                      return DropdownMenuItem(value: cat, child: Row(children: [Icon(style.icon, color: style.color, size: 16), const SizedBox(width: 8), Text(cat), if (model.incomeCats.contains(cat)) const Text(' (Entrata)', style: TextStyle(color: Colors.green, fontSize: 10))]));
                    }),
                  ],
                  onChanged: (value) {
                    setState(() {
  ...
  void _showImportSuccess() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Row(children: [Icon(Icons.check_circle, color: Colors.green), SizedBox(width: 8), Text('Importazione Completata')]),
        content: const Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.file_download_done, color: Colors.green, size: 48), SizedBox(height: 16), Text('Importazione completata con successo!', style: TextStyle(fontSize: 16), textAlign: TextAlign.center), SizedBox(height: 8), Text('Le transazioni sono state aggiunte al database', style: TextStyle(color: Colors.grey), textAlign: TextAlign.center)]),
        actions: [ElevatedButton(onPressed: () => Navigator.pop(context), child: const Text('Chiudi'))],
      ),
    );
  }
  ...
  List<String> _parseCSVFields(String line) {
    final fields = <String>[]; bool inQuotes = false; String current = '';

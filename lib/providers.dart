...
class MoneyModel extends ChangeNotifier {
  ...
  // Getter per tutte le categorie
  List<String> get allExpenseCats => [...expenseCats, ...customExpenseCats];
  List<String> get allIncomeCats => [...incomeCats, ...customIncomeCats];

  // Aggiungi categoria personalizzata
  void addCustomCategory(String name, IconData icon, bool isIncome, [Color? color]) {
    if (isIncome) {
  ...
  void deleteCustomCategory(String name, bool isIncome) {
    if (isIncome) {
  ...
  // ✅ CORRETTO: Ricorrenti partono dal mese successivo se il giorno è passato
List<MoneyTx> getRecurringTransactionsForPeriod(DateTime start, DateTime end) {
  ...
  for (var recurring in recurringTransactions) {
    ...
    while (currentMonth.isBefore(end) || (currentMonth.month == end.month && currentMonth.year == end.year)) {
      // ✅ Calcola la data della ricorrente per questo mese
      ...
      // ✅ LOGICA CORRETTA:
      // 1. Se la ricorrente è nel periodo richiesto (start-end)
      // 2. Se la data è <= oggi (non nel futuro)
      // 3. Se il giorno è già passato questo mese O se è il giorno corrente
      
      final today = DateTime(now.year, now.month, now.day);
      final recurringDay = DateTime(recurringDate.year, recurringDate.month, recurringDate.day);
      
      // ✅ Aggiungi SOLO se:
      // - La data ricorrente è nel range del periodo richiesto
      // - La data ricorrente è oggi O è già passata (non nel futuro)
      if (recurringDate.isAfter(start.subtract(const Duration(days: 1))) &&
          recurringDate.isBefore(end) &&
          (recurringDay.isBefore(today) || recurringDay.isAtSameMomentAs(today))) {
        
        // ✅ Evita duplicati: controlla se esiste già una transazione con stessa categoria/importo/data
  ...
  double get netWorth {
    final now = DateTime.now();
    final allTxs = [...transactions, ...getRecurringTransactionsForPeriod(DateTime(2020), now.add(const Duration(days: 365)))];
    return allTxs.fold(0.0, (sum, tx) {
      return sum + (tx.isIncome ? tx.amount : -tx.amount);
    });
  }

  // ✅ CORRETTO: Report mensile mostra SOLO transazioni reali (non future)
  ...
  Future<void> unpurchaseGoal(int goalId) async {
    ...
    final purchaseTxs = transactions.where(
      (tx) => tx.category == goal.title && !tx.isIncome && tx.amount == goal.target
    ).toList();
  ...
  // EXCEL IMPORT
  Future<Map<String, int>> importFromExcel(Uint8List fileBytes, Map<String, String> categoryMapping) async {
    try {
  ...
  // MMBACKUP IMPORT
  Future<Map<String, int>> importFromMMBackup(String jsonContent, Map<String, String> categoryMapping) async {
    try {
  ...
  MoneyTx? _parseMMBackupTransaction(Map txData, Map<String, String> categoryMapping) {
    try {
